...
from mobile_geodatabase import (
    GeoDatabase, Point, LineString, Polygon, MultiLineString,
    to_geojson_geometry
)
...
class TestGeoDatabase:
    def test_open_geodatabase(self, test_gdb):
        """Test opening a geodatabase file."""
        assert test_gdb.path == TEST_DB

    def test_list_tables(self, test_gdb):
        """Test listing tables."""
        tables = test_gdb.tables
        assert len(tables) > 0
    ...
    def test_table_info(self, test_gdb):
    ...
    def test_get_table(self, test_gdb):
        """Test getting table by name."""
        # Get first geometry table
        geom_tables = [t for t in test_gdb.tables if t.has_geometry]
        if geom_tables:
            table = test_gdb.get_table(geom_tables[0].name)
            assert table is not None
            assert table.name == geom_tables[0].name

    def test_get_table_case_insensitive(self, test_gdb):
...
class TestReadFeatures:
    def test_read_points(self, test_gdb):
        """Test reading point geometries."""
        # Find a point table
        point_tables = [t for t in test_gdb.tables
                       if t.geometry_type and 'Point' in t.geometry_type]
        if not point_tables:
    ...
    def test_read_lines(self, test_gdb):
        """Test reading line geometries."""
        # Find a line table
        line_tables = [t for t in test_gdb.tables
                      if t.geometry_type and 'Line' in t.geometry_type]
        if not line_tables:
    ...
    def test_read_polygons(self, test_gdb):
        """Test reading polygon geometries."""
        # Find a polygon table
        poly_tables = [t for t in test_gdb.tables
...
class TestCoordinateValidation:
    ...
    def test_all_geometries_valid(self, test_gdb):
        ...
        assert len(errors) == 0, f"Found invalid coordinates:\n" + "\n".join(errors[:10])


class TestGeoJsonOutput:
    def test_feature_to_geojson(self, test_gdb):
